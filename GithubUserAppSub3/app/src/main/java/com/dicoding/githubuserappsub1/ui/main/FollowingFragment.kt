...
import com.dicoding.githubuserappsub1.viewmodel.FollowingViewModel
...
class FollowingFragment : Fragment() {
    ...
    private var userFollowing: UserResponseItem? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        userFollowing = arguments?.getParcelable(DetailActivity.EXTRA_USER)

        if (userFollowing != null) {
            userFollowing?.let {
                followingViewModel.findUserFollowing(it.login)
            }
        }
    ...
    private fun showRecyclerView(listUserFollowing: List<UserResponseItem>) {
        ...
        listFollowingAdapter.setOnItemClickCallback(object : ListUserAdapter.OnItemClickCallback {
            override fun onItemClicked(user: UserResponseItem) {
                val intentDetail =
                    Intent(this@FollowingFragment.context, DetailActivity::class.java)
